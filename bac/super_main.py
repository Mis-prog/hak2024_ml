...
image=None
def path_your_photo(path):
    global image
    image = cv2.imread(path)

def method_name(path):
    global index_max, image_path
    global image
    path_your_photo(path)
    ...
    inform = res_data[res_data["id"] == index_max]
    return inform,image_path,cos_val
